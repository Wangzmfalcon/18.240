...
public partial class Station : System.Web.UI.Page
{
    ...
    private void GetData()
    {
        using (SqlConnection sqlcnn = new SqlConnection(sqlstr))
        {
            ...
            {
                sqlcmm.CommandText = "select * from MSAS_Station";
    ...
    protected void btnAdd_Click(object sender, EventArgs e)
    {
               using (SqlConnection sqlcnn1 = new SqlConnection(sqlstr))
        {
            ...
            {
                sqlcmm1.CommandText = "select Station from MSAS_Station where Station='" + txtName.Text + "'";
                ...
                else
                {
                    ...
                    using (SqlConnection sqlcnn = new SqlConnection(sqlstr))
                    {
                        using (SqlCommand sqlcmm = sqlcnn.CreateCommand())
                        {
                            sqlcmm.CommandText = "select max(ID) as ID1 from MSAS_Station";
                    ...
                    using (SqlConnection sqlcnn = new SqlConnection(sqlstr))
                    {
                        using (SqlCommand sqlcmm = sqlcnn.CreateCommand())
                        {
                            sqlcmm.CommandText = "insert into MSAS_Station(Station,ID)values(@title,@ID)";
    ...
    protected void GridView1_RowDeleting(object sender, GridViewDeleteEventArgs e)
    {
        ...
        using (SqlConnection sqlcnn = new SqlConnection(sqlstr))
        {
            using (SqlCommand sqlcmm = sqlcnn.CreateCommand())
            {
                sqlcmm.CommandText = "delete from MSAS_Station where ID=@Title";
    ...
    protected void GridView1_RowUpdating(object sender, GridViewUpdateEventArgs e)
    {
        ...
        using (SqlConnection sqlcnn = new SqlConnection(sqlstr))
        {
            using (SqlCommand sqlcmm = sqlcnn.CreateCommand())
            {
                sqlcmm.CommandText = "update MSAS_Station set Station=@Title where ID=@id";

...
public partial class Position : System.Web.UI.Page
{
    ...
    private void GetData()
    {
        using (SqlConnection sqlcnn = new SqlConnection(sqlstr))
        {
            ...
            {
                sqlcmm.CommandText = "select * from MSAS_Division";
    ...
    protected void btnAdd_Click(object sender, EventArgs e)
    {
        ...
        using (SqlConnection sqlcnn = new SqlConnection(sqlstr))
        {
            using (SqlCommand sqlcmm = sqlcnn.CreateCommand())
            {
                sqlcmm.CommandText = "select max(ID) as ID1 from MSAS_Division";
        ...
        using (SqlConnection sqlcnn = new SqlConnection(sqlstr))
        {
            using (SqlCommand sqlcmm = sqlcnn.CreateCommand())
            {
                sqlcmm.CommandText = "insert into MSAS_Division(Division,ID)values(@title,@ID)";
    ...
    protected void GridView1_RowDeleting(object sender, GridViewDeleteEventArgs e)
    {
        ...
        using (SqlConnection sqlcnn = new SqlConnection(sqlstr))
        {
            using (SqlCommand sqlcmm = sqlcnn.CreateCommand())
            {
                sqlcmm.CommandText = "delete from MSAS_Division where ID=@Title";
    ...
    {
        string username = (GridView1.Rows[e.RowIndex].FindControl("Label4") as TextBox).Text;

        using (SqlConnection sqlcnn = new SqlConnection(sqlstr))
        {
            using (SqlCommand sqlcmm = sqlcnn.CreateCommand())
            {
                sqlcmm.CommandText = "update MSAS_Division set Division=@Title where ID=@id";

...
public partial class Title : System.Web.UI.Page
{
    ...
    private void GetData()
    {
        using (SqlConnection sqlcnn = new SqlConnection(sqlstr))
        {
            ...
            {
                sqlcmm.CommandText = "select * from MSAS_Title";
    ...
    protected void btnAdd_Click(object sender, EventArgs e)
    {
               using (SqlConnection sqlcnn1 = new SqlConnection(sqlstr))
        {
            ...
            {
                sqlcmm1.CommandText = "select Title from MSAS_Title where Title='" + txtName.Text + "'";
                ...
                else
                {
                    ...
                    using (SqlConnection sqlcnn = new SqlConnection(sqlstr))
                    {
                        using (SqlCommand sqlcmm = sqlcnn.CreateCommand())
                        {
                            sqlcmm.CommandText = "select max(ID) as ID1 from MSAS_Title";
                    ...
                    using (SqlConnection sqlcnn = new SqlConnection(sqlstr))
                    {
                        using (SqlCommand sqlcmm = sqlcnn.CreateCommand())
                        {
                            sqlcmm.CommandText = "insert into MSAS_Title(Title,ID)values(@title,@ID)";
    ...
    protected void btnSeek_Click(object sender, EventArgs e)
    {
        using (SqlConnection sqlcnn = new SqlConnection(sqlstr))
        {
            using (SqlCommand sqlcmm = sqlcnn.CreateCommand())
            {
                sqlcmm.CommandText = "select * from MSAS_Title where Title=@MSAS_Title";
                sqlcmm.Parameters.AddWithValue("@MSAS_Title", this.txtName.Text);
                sqlcnn.Open();
                SqlDataAdapter adapter = new SqlDataAdapter(sqlcmm);
                DataTable dt = new DataTable();
    ...
    protected void GridView1_RowDeleting(object sender, GridViewDeleteEventArgs e)
    {
        ...
        using (SqlConnection sqlcnn = new SqlConnection(sqlstr))
        {
            using (SqlCommand sqlcmm = sqlcnn.CreateCommand())
            {
                sqlcmm.CommandText = "delete from MSAS_Title where ID=@Title";
    ...
    protected void GridView1_RowUpdating(object sender, GridViewUpdateEventArgs e)
    {
        string username = (GridView1.Rows[e.RowIndex].FindControl("TextBox2") as TextBox).Text;
            ...
            using (SqlConnection sqlcnn = new SqlConnection(sqlstr))
            {
                using (SqlCommand sqlcmm = sqlcnn.CreateCommand())
                {
                    sqlcmm.CommandText = "update MSAS_Title set title=@Title where ID=@id";

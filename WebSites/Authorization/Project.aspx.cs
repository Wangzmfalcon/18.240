...
public partial class Project : System.Web.UI.Page
{
    ...
    private void GetData()
    {
        using (SqlConnection sqlcnn = new SqlConnection(sqlstr))
        {
            ...
            {
                sqlcmm.CommandText = "select * from MSAS_Project";
    ...
    protected void btnAdd_Click(object sender, EventArgs e)
    {
               using (SqlConnection sqlcnn1 = new SqlConnection(sqlstr))
        {
            ...
            {
                sqlcmm1.CommandText = "select Project from MSAS_Project where Project='" + txtName.Text + "'";
                ...
                else
                {
                    ...
                    using (SqlConnection sqlcnn = new SqlConnection(sqlstr))
                    {
                        using (SqlCommand sqlcmm = sqlcnn.CreateCommand())
                        {
                            sqlcmm.CommandText = "select max(ID) as ID1 from MSAS_Project";
                    ...
                    using (SqlConnection sqlcnn = new SqlConnection(sqlstr))
                    {
                        using (SqlCommand sqlcmm = sqlcnn.CreateCommand())
                        {
                            sqlcmm.CommandText = "insert into MSAS_Project(Project,ID,Remark)values(@title,@ID,@Remark)";
                            sqlcmm.Parameters.AddWithValue("@title", this.txtName.Text);
                            sqlcmm.Parameters.AddWithValue("@ID", num);
                            sqlcmm.Parameters.AddWithValue("@Remark", this.txtremark.Text);
                            sqlcnn.Open();
    ...
    protected void GridView1_RowDeleting(object sender, GridViewDeleteEventArgs e)
    {
        ...
        using (SqlConnection sqlcnn = new SqlConnection(sqlstr))
        {
            using (SqlCommand sqlcmm = sqlcnn.CreateCommand())
            {
                sqlcmm.CommandText = "delete from MSAS_Project where ID=@Title";
    ...
    protected void GridView1_RowUpdating(object sender, GridViewUpdateEventArgs e)
    {
        ...
        string Remark = (GridView1.Rows[e.RowIndex].FindControl("TextBoxRemark") as TextBox).Text;

        using (SqlConnection sqlcnn = new SqlConnection(sqlstr))
        {
            using (SqlCommand sqlcmm = sqlcnn.CreateCommand())
            {
                sqlcmm.CommandText = "update MSAS_Project set Project=@Title,Remark=@Remark where ID=@id";
                sqlcmm.Parameters.AddWithValue("@Title", username);
                sqlcmm.Parameters.AddWithValue("@Remark", Remark);
